...
const ArticleComponent = (props: Props) => {
    const {article, handleUpdateArticle, handleDeleteArticle, handleSubmitArticle} = props;
                ...
                <div className="mockup-window bg-base-300">
                    ...
                    <div className={"bg-base-200 flex flex-col justify-center px-4"}>
                        ...
                        <div className={"flex flex-row ml-auto gap-4"}>
                            <button disabled={!article} onClick={()=>article?.id && handleDeleteArticle(article.id)} className="btn btn-secondary text-xl my-4 ml-auto">Remove article
                            </button>
                            <button onClick={()=>!!article ? handleUpdateArticle({title:titleInput, content: contentInput, id: article.id}) : handleSubmitArticle({title: titleInput, content: contentInput})} className="btn btn-primary text-xl my-4 ml-auto">{!!article ? "Modify Article" : "Add Article"}
                            </button>
                        </div>
                    </div>
                </div>
